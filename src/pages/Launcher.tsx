...
export default () => {
  ...
  if (config.i18n !== 'en-US') config.onChangeLocalization('en-US');
  ...
  useEffect(() => {
    ...
    if (instance.FS.analyzePath(`${instance.ENV.HOME}/id1`).exists) return setHasData(true);
    ...
    fetch(gameData)
      ...
      .then(blob => zipInputReader(`${instance.ENV.HOME}`, instance, blob))
      .then(setHasData)
      .catch(error => {
  ...
  const runInstance = () => {
    ...
    try {
      instance.callMain(['-basedir', `${instance.ENV.HOME}`, '-game', 'travail']);
    }
    catch (e) {
      console.log(e);
    }
  ...
  return (
    ...
    >
      ...
      <CardContent sx={{
        ...
        <canvas id="canvas" ref={canvas} width={1280} height={720} style={{
          width: '100%', height: '100%', position: 'absolute', zIndex: 100,
          top: '50%', left: '50%', transform: 'translate(-50%, -50%)',
          maxWidth: 'calc(100vh * 1280 / 720)'
        }}></canvas>
      </CardContent>
